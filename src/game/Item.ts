...
export class Item {
  ...
  constructor(config: ItemConfig = {}) {
    ...
    this.slot = config.slot !== undefined ? config.slot : null; // Equipment slot if equippable
    ...
    this.charges = config.charges !== undefined ? config.charges : null; // For items with limited uses
    this.maxCharges = config.maxCharges !== undefined ? config.maxCharges : null;
   ...
   */
  canEquipToSlot(slot: string): boolean {
   ...
   * Load from JSON
   */
  static fromJSON(data: ItemJSON | ItemConfig): Item {
